'use strict'
let fs=require('fs'),net=require('net'),os=require('os'),path=require('path'),cp=require('child_process')
let log
{
  // record no more than N log messages per T milliseconds; at any moment, there have been n messages since time t
  let stdoutOK=1,N=500,T=1000,n=0,t=0,t0=+new Date,l=[] // l:listeners
  log=(s)=>{
    let t1=+new Date;if(t1-t>T){t=t1;n=1} // if last message was too long ago, start counting afresh
    ...
    let l1=l.slice(0);for(let i=0;i<l1.length;i++)l1[i](m) // notify listeners, prevent concurrent modification
  }
  log.addListener=(x)=>{l.push(x)}
  log.rmListener=(x)=>{let i=l.indexOf(x);i>=0&&l.splice(i,1)}
  let f=process.env.RIDE_LOG
  if(f){ // if $RIDE_LOG is present, also log to a file (in addition to stdout)
    let h=process.env.HOME||process.env.USERPROFILE;if(h)f=path.resolve(h,f)
    fd=fs.openSync(f,'a');l.push((s)=>{let b=Buffer(m);fs.writeSync(fd,b,0,b.length)})
  }
...
let trunc=(s)=>s.length>1000?s.slice(0,997)+'...':s
let ls=(x)=>fs.readdirSync(x)
let sil=(f)=>(x)=>{try{f(x)}catch(_){}} // exception silencer
let shEsc=(x)=>"'"+x.replace(/'/g,"'\\''")+"'" // shell escape
let parseVer=(s)=>s.split('.').map((x)=>+x)
let send=(s)=>{if(clt){log('send '+trunc(s));let b=Buffer('xxxxRIDE'+s);b.writeInt32BE(b.length,0);clt.write(b)}}
let cmd=(c,h)=>{send(JSON.stringify([c,h||{}]))} // c:command name, h:arguments as a JS object
let toBrowser=(x,y)=>{log('to browser:'+trunc(JSON.stringify([x,y])));skt&&skt.emit(x,y)}
let initInterpreterConn=()=>{
  let P=process.env.RIDE_PROTOCOL||'2'
  ...
  clt.on('data',(x)=>{
    ...
    while(q.length>=4&&(n=q.readInt32BE(0))<=q.length){
      ...
      let m=''+q.slice(8,n);q=q.slice(n);log('recv '+trunc(m))
      if(m[0]==='['){
        let u=JSON.parse(m);skt&&skt.emit(u[0],u[1])
...
let handlers={
  '*connect':(x)=>{
    ...
    clt=m.connect(o,()=>{
      if(x.ssl&&x.subj){
        let s=clt.getPeerCertificate().subject.CN
        if(s!==x.subj){
          toBrowser('*error',{msg:
  ...
  '*launch':(x)=>{
    let exe=(x||{}).exe||process.env.RIDE_INTERPRETER_EXE||'dyalog'
    srv=net.createServer((c)=>{
      log('spawned interpreter connected');let a=srv.address();srv&&srv.close();srv=null;clt=c
    ...
    srv.listen(0,'127.0.0.1',()=>{
      let a=srv.address(),hp=a.address+':'+a.port
      ...
      try{
        let h=x.env||{},H=process.env;for(let k in H)h[k]=H[k];h.RIDE_INIT='CONNECT:'+hp;h.RIDE_SPAWNED='1'
  ...
  '*sshFetchListOfInterpreters':(x)=>{
    let c=sshExec(x,'/bin/ls /opt/mdyalog/*/*/*/mapl /Applications/Dyalog-*/Contents/Resources/Dyalog/mapl',(sm)=>{
      let s=''
  ...
  '*getProxyInfo':()=>{
    ...
    let r={interpreters:[],platform:process.platform} // proxy info (the result)
    if(/^win/.test(r.platform)){
      ...
      try{
        let a='/Applications'
        ls(a).forEach((x)=>{
          let m=/^Dyalog-(\d+\.\d+)\.app$/.exec(x), exe=a+'/'+x+'/Contents/Resources/Dyalog/mapl'
...
let sshExec=(x,cmd,callback)=>{
  try{ // see https://github.com/mscdex/ssh2/issues/238#issuecomment-87495628 for why we use tryKeyboard:true
    let c=new(require('ssh2').Client),o={host:x.host,port:x.port,username:x.user,tryKeyboard:true}
...
module.exports=(x)=>{
  ...
  ;(skt=x).onevent=(x)=>{
    let f=handlers[x.data[0]];f&&log('from browser:'+trunc(JSON.stringify(x.data)))
