...
,initInterpreterConn=_=>{
  ...
  clt.on('error',x=>{toBrowser('*error',{msg:''+x});clt=0})
  clt.on('end',_=>{log('interpreter diconnected');toBrowser('*disconnected');clt=0})
...
,handlers={
  '*connect':x=>{
    ...
    clt.on('error',x=>{log('connect failed: '+x);clt=0;toBrowser('*error',{msg:x.message})})
  },
  '*launch':x=>{
    ...
    srv.on('error',x=>{log('listen failed: '+x);srv=clt=0;toBrowser('*error',{msg:x.message})})
    srv.listen(0,'127.0.0.1',_=>{
      ...
      child.on('error',x=>{
        ...
        toBrowser('*error',{code:x.code,msg:x.code==='ENOENT'?"Cannot find the interpreter's executable":''+x})
      })
      child.on('exit',(code,sig)=>{srv&&srv.close();srv=clt=0;toBrowser('*spawnedExited',{code,sig});child=0})
    })
  },
  ...
  '*ssh':x=>{
    const c=sshExec(x,'/bin/sh',sm=>{
      sm.on('close',(code,sig)=>{toBrowser('*sshExited',{code,sig});c.end()})
...
module.exports=x=>{
  ;(skt=x).recv=(x,y)=>{const f=handlers[x],s=JSON.stringify([x,y])
                        f&&log('from browser:'+trunc(s));f?f(y):sendEach([s])}
  log('browser connected');child&&toBrowser('*spawned',{pid:child.pid})
}
