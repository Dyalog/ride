...
if [ -f /usr/bin/dyalog ]; then
        ...
        cat /usr/share/applications/dyalog-tty.desktop | sed 's/^Exec=.*/Exec=env TERM=xterm RIDE_SPAWN=\/usr\/bin\/dyalog \/usr\/bin\/ride-${BASE_VERSION}/' > /usr/share/applications/dyalog.desktop
