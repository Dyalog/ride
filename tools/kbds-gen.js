...
get('dfns.dyalog.com','/n_keyboards.htm',function(data0){
  get('dfns.dyalog.com','/n_kbmac.htm',function(data1){
    var geom={_:'iso'},layouts={}
    ;[data0,data1].forEach(function(data){
      cheerio.load(data)('pre').text()
        .replace(/\r\n/g,'\n')
        .replace(/\nDyalog( Mac)? APL\/([A-Z]{2}).*\n[¯\-]*\n(┌(?:.*\n){12}.*)/gm,function(_,mac,lc,desc){
          if(mac)lc+='-Mac'
    ...
    fs.writeFileSync('../client/kbds.js',
      ...
      '  '+Object.keys(layouts).sort().map(function(lc){
             var l=layouts[lc];return JSON.stringify(lc)+':[\n   '+l.map(JSON.stringify).join(',\n   ')+'\n  ]'
           }).join(',\n  ')+'\n'+
      '};\n'
    )
  })
