...
const rq=require,express=rq('express'),fs=rq('fs'),WSServer=rq('ws').Server,{spawn}=rq('child_process')
  ,_d=__dirname,t0=+new Date,log=x=>{process.stdout.write(`${new Date-t0}: ${x}\n`)}
let tid,busy
const build=()=>{
...
if(process.env.RIDE_SERVER_WATCH){
  ;['.','src','style','style/img','style/thm'].forEach(x=>{fs.watch(_d+'/'+x,()=>{tid=tid||setTimeout(build,100)})})
  build()
}
...
,port=insecure?8000:8443
,server=insecure?rq('http').createServer(app)
                :rq('https').createServer({cert:fs.readFileSync('ssl/cert.pem'),key:fs.readFileSync('ssl/key.pem')},app)
;(new WSServer({server})).on('connection',ws=>{
