...
The RIDE provides several mechanisms that assist with accuracy and provide clarity when entering expressions in a Dyalog Session, including, [paired enclosures](paired_enclosures.md), [autocomplete](autocomplete.md), [context-sensitive help](context_sensitive_help.md) and [syntax clouring](syntax_colouring.md).
...
When an opening enclosure character is entered, the RIDE automatically includes its closing pair. This reduces the risk of an invalid expression being entered due to unbalanced enclosures. This feature can be disabled in the [General tab](general_tab.md) of the Preferences dialog box.
...
This feature can be disabled or customised in the [General tab](general_tab.md) of the Preferences dialog box.
...
The foreground colour and/or the background colour (highlighting) can be customised by syntax type – see [Colours Tab](colours_tab.md).
...
The number of threads currently in use is displayed in the [Status bar](status_bar.md).

## Suspending Execution

To assist with investigations into the behaviour of a set of statements (debugging), the system can be instructed to suspend execution just before a particular statement. This is done by setting a [breakpoint](breakpoints.md).

It is sometimes necessary to suspend the execution of a function, for example, if an  endless loop has been inadvertently created or a response is taking an unacceptably long time. This is done using an [interrupt](interrupts.md).
...
- Enter:
    ...
    - Microsoft Windows: <kbd>Ctrl</kbd> + <kbd>Q</kbd> (only if the Dyalog Unicode IME is not enabled – see [Keyboard Tab](keyboard_tab.md#))
