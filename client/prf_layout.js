D.modules.prf_layout=function(require){'use strict'

var prf=require('./prf'),layouts=require('./kbds').layouts,geom=require('./kbds').geom
...
this.init=function($e){
  ...
  if(!layouts[prf.kbdLocale()]){
    ...
    prf.kbdLocale(D.mac&&layouts[l+'_'+c+'_Mac']?l+'_'+c+'_Mac':layouts[l+'_'+c]?l+'_'+c:d?d:'en_US')
  }
  $('#layout-rst').click(function(){
    var lc=$lc.val();$pfx.val(prf.prefixKey.getDefault()).change()
...
this.load=function(){
  $lc.val(prf.kbdLocale());$pfx.val(prf.prefixKey()).change();model={}
  for(var lc in layouts){var l=layouts[lc];model[lc]=[l[2].split(''),l[3].split('')]}
  var pm=prf.prefixMaps()
  for(var lc in pm){
    var v=pm[lc]
  ...
  updGlyphs();D.win&&$('#layout-ime').prop('checked',!!prf.ime())
}
...
this.save=function(){
  prf.prefixKey($pfx.val());prf.kbdLocale($lc.val())
  var h={}
  for(var lc in model){
  ...
  prf.prefixMaps(h);D.win&&prf.ime($('#layout-ime').is(':checked'))
}
