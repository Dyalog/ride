$.fn.splitter=function(args){'strict mode' // methvin.com/splitter
  ...
  return this.each(function(){
    function startSplit(e){
      $p.css('-webkit-user-select','none');$m.addClass(A);$a._posSplit=$a[0][ohw]-e[pyx]
      $(document).bind('mousemove',doSplit).bind('mouseup',endSplit)
    }
    function doSplit(e){resplit($a._posSplit+e[pyx])}
    function endSplit(e){
      $m.removeClass(A);$p.css('-webkit-user-select','text')
    ...
    function resplit(p){
      ...
      $m._DA=$m[0][ohw] // $m size may change during dock
    ...
    $m._DA=$m[0][ohw]
    $s._PBF=$s._PBA=0;$a._pane=TL;$b._pane=BR
