...
'use strict'
let compression=require('compression'),express=require('express'),fs=require('fs'),http=require('http'),
    https=require('https'),WSServer=require('ws').Server,proxy=require('./proxy'),{spawn}=require('child_process')
let t0=+new Date,log=(s)=>{process.stdout.write((new Date-t0)+': '+s+'\n')}
let _d=__dirname,tid,busy
let build=()=>{
...
let insecure=process.env.RIDE_SERVER_INSECURE
let port=insecure?8000:8443
let server=insecure?http.createServer(app)
...
;(new WSServer({server:server})).on('connection',(ws)=>{
  let l={},io={} // l:listeners, io:socket.io-like interface
  io.emit=(x,y)=>{ws.send(JSON.stringify([x,y]));return io}
  io.on=(e,f)=>{(l[e]=l[e]||[]).push(f);return io}
  io.onevent=(x)=>{let a=l[x.data[0]]||[];for(let i=0;i<a.length;i++)a[i].apply(null,x.data.slice(1))}
