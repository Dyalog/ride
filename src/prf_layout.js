...
D.prf_tabs.push({
  ...
  init:function($e){
    ...
    $('#layout-rst').click(function(){
      var lc=$lc.val();$pfx.val(D.prf.prefixKey.getDefault()).change()
  ...
  load:function(){
    $lc.val(D.prf.kbdLocale());$pfx.val(D.prf.prefixKey()).change();model={}
  ...
  validate:function(){if($pfx.val().length!==1)return{msg:'Invalid prefix key',el:$pfx}},
  save:function(){
    D.prf.prefixKey($pfx.val());D.prf.kbdLocale($lc.val())
    var h={}
    for(var lc in model){
