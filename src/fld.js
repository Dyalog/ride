...
CM.defineOption('foldGutter',false,function(cm,v,o){ //v:new value, o:old value
  ...
  if(v){cm.state.foldGutter={from:0,to:0};eh.swapDoc(cm);for(var k in eh)cm.on(k,eh[k])}
})
...
function updMarkers(cm,a,b){ //a..b:from..to line - ignored, we always update the whole editor
  //there's room for optimisation here though performance seems good enough for now
  a=0;b=cm.lineCount();var f=cm.foldOption({},'rangeFinder'),e=[],i=a //e:ends, i:current line
  cm.eachLine(a,b,function(lh){var t='folded' //t:marker type (folded,open,end,cont with corresponding css classes)
...
eh.change=function(cm){var h=cm.state.foldGutter;if(!h)return
                       h.from=h.to=0;clearTimeout(h.tid);h.tid=setTimeout(function(){eh.swapDoc(cm)},600)}
eh.viewportChange=function(cm){
  var h=cm.state.foldGutter;if(!h)return
  clearTimeout(h.tid);h.tid=setTimeout(function(){
    var vp=cm.getViewport(), c=h.from===h.to||vp.from-h.to>20||h.from-vp.to>20
    c?eh.swapDoc(cm):cm.operation(function(){if(vp.from<h.from){updMarkers(cm,vp.from,h.from);h.from=vp.from}
