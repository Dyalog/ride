...
D.IDE = function () {
  ...
  ide.handlers = { //for RIDE protocol messages
    Identify: function (x) {
    D.remoteIdentification = x; ide.updTitle(); ide.connected = 1; ide.wins[0].updPW(1)
    ...
    OpenWindow: function (ee) {
      ...
      if (bro) { //add next to existing editor
        ...
        if (p.type !== t0) {
          var q = gl.createContentItem({ type: t0 }, p); p.parent.replaceChild(p, q)
    ...
    OptionsDialog: function (x) {
      ...
      if (D.el && process.env.RIDE_NATIVE_DIALOGS) {
        ...
        var ret = function (r) {
          I.gd_btns.onclick = I.gd_close.onclick = null; I.gd.hidden = 1
          D.send('ReplyOptionsDialog', { index: r, token: x.token })
          D.ide.focusedWin.focus();
        }
        I.gd_close.onclick = function () { ret(-1) }
        I.gd_btns.onclick = function (e) {
    ...
    StatusOutput: function (x) {
      ...
      if (!w) {
        w = ide.wStatus = new D.el.BrowserWindow({ width: 600, height: 400, parent: D.elw })
  ...
  $(I.lb_inner).sortable({
    forcePlaceholderSize: 1, placeholder: 'lb_placeholder', revert: 1, distance: 8,
  ...
  D.prf.ilf(function (x) { var i = x ? -1 : D.prf.indent(); eachWin(function (w) { if (w.id) { w.cm.setOption('smartIndent', i >= 0); w.cm.setOption('indentUnit', i); } }) })
  D.prf.indent(function (x) { var i = D.prf.ilf() ? -1 : x; eachWin(function (w) { if (w.id) { w.cm.setOption('smartIndent', i >= 0); w.cm.setOption('indentUnit', i); } }) })
  ...
  var togglePanel = function (comp_name, comp_title, left) {
    ...
    if (p.type !== 'row') {
      var row = gl.createContentItem({ type: 'row' }, p); p.parent.replaceChild(p, row)
...
D.IDE.prototype = {
  ...
  updTitle: function () { //change listener for D.prf.title
    ...
    document.title = D.prf.title().replace(/\{\w+\}/g, function (x) {
...
CM.commands.ZM = function () {
  var w = D.ide.focusedWin; w.container.parent.toggleMaximise()
