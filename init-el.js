var fs=require('fs'),os=require('os'),path=require('path'),spawn=require('child_process').spawn,
...
var D=global.D={};D.nwjs=0;D.el=1;D.win=/^win/i.test(ps.platform);D.mac=ps.platform=='darwin';D.floating=0
...
var mkdirs=(x)=>{if(!fs.existsSync(x)){mkdirs(path.dirname(x));fs.mkdirSync(x)}}
;(()=>{
  ...
  D.db={ // file-backed storage with an API similar to that of localStorage
    key:(i)=>k[i],
    getItem   :(x)=>  {var i=k.indexOf(x);return i<0?null:v[i]},
    setItem   :(x,y)=>{var i=k.indexOf(x);if(i<0){k.push(x);v.push(y)}else{v[i]=y};dbWrite()},
    removeItem:(x)=>  {var i=k.indexOf(x);if(i>=0){k.splice(i,1);v.splice(i,1);dbWrite()}},
    _getAll:()=>{var r={};for(var i=0;i<k.length;i++)r[k[i]]=v[i];return r}
  }
  Object.defineProperty(D.db,'length',{get:()=>k.length})
  var d=env.HOME+'/.config/ride40',f=d+'/prefs.json';mkdirs(d)
  try{if(fs.existsSync(f)){var h=JSON.parse(fs.readFileSync(f,'utf8'));for(var x in h){k.push(x);v.push(h[x])}}}
  catch(e){console.error(e)}
  var st=0,dbWrite=()=>{ // st: state 0=initial, 1=write pending, 2=write in progress
...
//  // createSocket moved to client/init.js
